import torch
from scripts.SC2_PCR.common import knn, rigid_transform_3d
from scripts.SC2_PCR.utils.SE3 import transform
import numpy as np
...
class Matcher():
    ...
    def cal_seed_trans(self, seeds, SC2_measure, src_keypts, tgt_keypts):
        ...
        return final_trans, seedwise_fitness
    ...
    def SC2_PCR(self, src_keypts, tgt_keypts):
        ...
        sc_timer, select_timer, sc2_timer, seed_timer, refine_timer = Timer(), Timer(), Timer(), Timer(), Timer()
        ...
        #################################
        final_trans, seedwise_fitness = self.cal_seed_trans(seeds, SC2_measure, src_keypts, tgt_keypts)
        seed_timer.toc()
        ...
        # print(f"SC time: {sc_timer.avg}, Seed Selection time: {select_timer.avg}," +
        #       f" SC2 time: {sc2_timer.avg}, Seed Calculation time: {seed_timer.avg}," +
        #       f" Refinement time: {refine_timer.avg}", flush=True)
    ...
    def estimator(self, src_keypts, tgt_keypts, src_features, tgt_features):
        ...
        #################################
        pred_trans, seedwise_fitness  = self.SC2_PCR(src_keypts_corr, tgt_keypts_corr)
        ...
        return pred_trans, pred_labels, src_keypts_corr, tgt_keypts_corr, seedwise_fitness
